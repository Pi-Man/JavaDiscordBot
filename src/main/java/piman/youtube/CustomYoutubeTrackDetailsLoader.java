...
public class CustomYoutubeTrackDetailsLoader implements YoutubeTrackDetailsLoader {
  private static final Logger log = LoggerFactory.getLogger(CustomYoutubeTrackDetailsLoader.class);
  ...
  private YoutubeTrackDetails load(
    ...
    try {
      ...
      Captions captions = getCaptions(httpInterface, finalData);
      
      return new CustomYoutubeTrackDetails(videoId, finalData, captions);
  ...
  private Captions getCaptions(HttpInterface httpInterface, YoutubeTrackJsonData finalData) throws IOException {
      if (!finalData.playerResponse.get("captions").isNull()) {
		JsonBrowser captions = finalData.playerResponse.get("captions").get("playerCaptionsTracklistRenderer");
